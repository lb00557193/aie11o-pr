...
2. 從最新 commit subject + git diff 自動生成 PR title 和 description（中文，照 `pr_generation` 模板：Why / How / Implement / 測試驗證 / 補充說明 / 相關連結）
3. 建 PR
4. 把 description 同步成 Jira comment（含一個隱形 marker `<!-- claude-pr:AOX-XXXX -->`）。重跑同 ticket 時會用 prior PR description + 整段 feature 完整 diff 重新生成 holistic 描述，PUT 覆寫單一條 comment（不會多生）。
